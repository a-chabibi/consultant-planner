  ...
  const handleSave = async (force=false)=>{
    ...
    try{
      ...
      if(!force){
        ...
        if(c.hasConflict){ setConflictWarning({ conflicts: c.conflicts, dates: datesToSave }); return; }
      }
      if(editing){
  ...
  return (
    <div className="min-h-screen bg-[#fcfbf8] p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        {/* HEADER MODERN */}
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8 bg-white p-5 rounded-[24px] border shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-zinc-900 text-white rounded-xl flex items-center justify-center font-bold">S</div>
            <div>
              <h1 className="text-xl font-bold">Dashboard Konsultan</h1>
              <p className="text-xs text-zinc-500">Jadwal Konsultan SPRINT</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="px-3 py-2 bg-zinc-50 border rounded-xl text-xs max-w-[200px] truncate">{userEmail}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-white p-6 rounded-[24px] border shadow-sm flex items-center gap-4">
            <div className="w-14 h-14 bg-[#6366f1] rounded-2xl flex items-center justify-center text-white text-2xl">📅</div>
            <div><div className="text-xs text-zinc-500">Jadwal Bulan Ini</div><div className="text-3xl font-bold">{schedules.length}</div></div>
          </div>
          <div className="bg-white p-6 rounded-[24px] border shadow-sm flex items-center gap-4">
            <div className="w-14 h-14 bg-[#fef3c7] rounded-2xl flex items-center justify-center text-xl">⏳</div>
            <div><div className="text-xs text-zinc-500">Menunggu</div><div className="text-3xl font-bold">{pendingCount}</div></div>
          </div>
          <div className="bg-white p-6 rounded-[24px] border shadow-sm flex items-center gap-4">
            <div className="w-14 h-14 bg-[#dcfce7] rounded-2xl flex items-center justify-center text-xl">✅</div>
            <div><div className="text-xs text-zinc-500">Disetujui</div><div className="text-3xl font-bold">{approvedCount}</div></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-6">
          ...
          <div className="bg-white rounded-[24px] border shadow-sm p-6">
            ...
            <div className="grid grid-cols-7 gap-px bg-zinc-100 rounded-2xl overflow-hidden border">
              {["Min","Sen","Sel","Rab","Kam","Jum","Sab"].map(d=><div key={d} className="bg-zinc-50 text-center text-[11px] font-semibold text-zinc-500 py-3">{d}</div>)}
              {days.map((d,idx)=>{
                if(d===null) return <div key={idx} className="bg-white min-h-[100px]"/>;
                ...
                return (
                  <div key={idx} onClick={()=>{ setSelectedDate(dateStr); setForm({ client_name:"", project_name:"", location:"WFO" as any, task_description:"", start_time:"09:00", end_time:"17:00"}); setEditing(null); setRepeatCount(1); setShowForm(true); }}
                    className={`bg-white min-h-[100px] p-2 cursor-pointer hover:bg-zinc-50 transition relative ${isToday?"ring-2 ring-zinc-900 ring-inset":""} ${isConflict?"bg-red-50":""}`}>
                    <div className="flex justify-between items-start"><span className={`text-sm font-medium w-6 h-6 flex items-center justify-center rounded-full ${isToday?"bg-zinc-900 text-white":""}`}>{d}</span>{isConflict && <span className="text-[10px] bg-red-500 text-white px-1.5 rounded-full">Conflict</span>}</div>
                    <div className="mt-2 space-y-1">
                      {list.slice(0,2).map(s=><div key={s.id} className={`text-[10px] px-1.5 py-0.5 rounded-full truncate font-medium ${s.status==="PENDING"?"bg-yellow-100 text-yellow-800":s.status==="APPROVED"?"bg-emerald-100 text-emerald-800":"bg-red-100 text-red-800"}`}>{s.client_name}</div>)}
                      {list.length>2 && <div className="text-[10px] text-zinc-400">+{list.length-2} lagi</div>}
                    </div>
                  </div>
                );
              })}
            </div>
            <div className="flex gap-4 mt-4 text-xs"><span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-yellow-400 rounded-full"/>Meeting</span><span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-emerald-500 rounded-full"/>Approved</span><span className="flex items-center gap-1.5"><span className="w-3 h-3 bg-red-500 rounded-full"/>Conflict</span></div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3 max-h-[700px] overflow-auto pr-1">
              {schedules.slice(0,10).map(s=>(
                <div key={s.id} className="bg-white p-4 rounded-[20px] border shadow-sm hover:shadow-md transition">
                  <div className="flex justify-between items-start">
                    <div><div className="font-bold text-sm">{s.client_name}</div><div className="text-xs text-zinc-500">Project: {s.project_name}</div></div>
                    <span className={`text-[11px] px-2.5 py-1 rounded-full font-medium ${s.status==="PENDING"?"bg-yellow-100 text-yellow-800":s.status==="APPROVED"?"bg-emerald-100 text-emerald-800":"bg-red-100 text-red-800"}`}>{s.status}</span>
                  </div>
                  <div className="mt-3 space-y-1.5 text-xs">
                    <div className="flex items-center gap-2 bg-zinc-50 p-2 rounded-xl"><span>📅</span> {formatTanggalHari(s.date)}</div>
                    <div className="flex items-center gap-2"><span>🕘</span> {s.start_time}-{s.end_time} <span className="ml-2 px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-[10px]">{s.location}</span></div>
                    {s.task_description && <div className="text-zinc-500 text-[11px] line-clamp-2">{s.task_description}</div>}
                    {s.rejection_reason && <div className="text-red-600 bg-red-50 p-2 rounded-xl text-[11px]">Alasan ditolak: {s.rejection_reason}</div>}
                  </div>
                  {(s.status==="PENDING"||s.status==="REJECTED") && (
                    <div className="flex gap-2 mt-3">
                      <button onClick={()=>{ setEditing(s); setSelectedDate(s.date); setForm({ client_name:s.client_name, project_name:s.project_name, location:s.location as any, task_description:s.task_description, start_time:s.start_time, end_time:s.end_time}); setShowForm(true); }} className="flex-1 border rounded-xl py-2 text-xs font-medium hover:bg-zinc-50">✏️ Edit</button>
                      <button onClick={async()=>{ if(confirm("Hapus jadwal ini?")){ await supabase.from("schedules").delete().eq("id", s.id); load(); } }} className="flex-1 bg-red-50 text-red-600 border border-red-200 rounded-xl py-2 text-xs font-medium hover:bg-red-100">🗑️ Hapus</button>
                    </div>
                  )}
                </div>
              ))}
              {schedules.length===0 && <div className="bg-white p-10 rounded-[20px] border text-center text-zinc-400 text-sm">Belum ada jadwal bulan ini. Klik tanggal di kalender untuk tambah.</div>}
            </div>
          </div>
        </div>
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-[24px] w-full max-w-lg p-6 max-h-[90vh] overflow-auto shadow-2xl">
            <h3 className="font-bold text-lg">{editing?"Edit":"Tambah"} Jadwal</h3>
            <p className="text-sm text-zinc-500">{selectedDate? formatTanggalHari(selectedDate) : ""}</p>
            <div className="mt-5 space-y-3">
              <input value={form.client_name} onChange={e=>setForm({...form, client_name:e.target.value})} placeholder="Nama Client" className="w-full border rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-zinc-900 focus:outline-none"/>
              <input value={form.project_name} onChange={e=>setForm({...form, project_name:e.target.value})} placeholder="Nama Project" className="w-full border rounded-2xl px-4 py-3 text-sm focus:ring-2 focus:ring-zinc-900 focus:outline-none"/>
              ...
              <textarea value={form.task_description} onChange={e=>setForm({...form, task_description:e.target.value})} placeholder="Deskripsi tugas" className="w-full border rounded-2xl px-4 py-3 text-sm min-h-[80px]"/>
            ...
            <div className="flex gap-3 mt-6"><button disabled={isSaving} onClick={()=>setShowForm(false)} className="flex-1 border py-3 rounded-2xl text-sm disabled:opacity-50">Batal</button><button disabled={isSaving} onClick={()=>handleSave()} className="flex-1 bg-zinc-900 text-white py-3 rounded-2xl text-sm font-medium disabled:opacity-50">{isSaving? "Menyimpan..." : (editing? "Update" : (repeatCount>1? `Simpan ${repeatCount} Hari` : "Simpan"))}</button></div>
          </div>
        </div>
      )}
      ...
      {conflictWarning && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-[24px] w-full max-w-md p-6 border-2 border-red-200 shadow-2xl">
            <h3 className="font-bold text-red-600">⚠ Bentrok Terdeteksi!</h3>
            <p className="text-sm mt-2 text-zinc-600">Kamu sudah ada jadwal di tanggal ini:</p>
