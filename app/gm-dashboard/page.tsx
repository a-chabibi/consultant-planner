...
export default function GMDashboard(){
  ...
  const perKonsultan: [string, number][] = (()=> {
    ...
    schedules.forEach(s=>{
      const email = (s.profiles?.email as string) || getEmail(s.consultant_id);
      if(email && email!=="-") map[email]=(map[email]||0)+1;
    });
    return Object.entries(map).sort((a,b)=>b[1]-a[1]).slice(0,6) as [string, number][];
  })();

  const maxCount = perKonsultan.length>0? Math.max(...perKonsultan.map(p=>p[1])) : 1;
  ...
  const detailData: Schedule[] = (()=> {
    if(!selectedCard) return [];
    if(selectedCard.type==="pending") return pending;
    if(selectedCard.type==="approved") return approved;
    if(selectedCard.type==="conflict") {
      return Array.from(conflictIds).map(id=>schedules.find(s=>s.id===id)).filter(Boolean) as Schedule[];
    }
    return schedules;
  })();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f0fdf4] via-[#ecfdf5] to-[#d1fae5] p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8 bg-white/80 backdrop-blur-xl p-6 rounded- border border-emerald-100 shadow-[0_20px_60px_-20px_rgba(16,185,129,0.3)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
          <div onClick={()=>setSelectedCard({ type:"pending", title:"Butuh Approval - Menunggu Persetujuan" })} className="group cursor-pointer bg-gradient-to-br from-amber-50 to-yellow-100 p-7 rounded- border border-amber-200/50 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all">
          ...
          <div onClick={()=>setSelectedCard({ type:"total", title:"Semua Jadwal Bulan Ini" })} className="group cursor-pointer bg-white/80 backdrop-blur p-7 rounded- border border-emerald-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all">
          ...
          <div onClick={()=>setSelectedCard({ type:"approved", title:"Jadwal Selesai / Disetujui" })} className="group cursor-pointer bg-white/80 backdrop-blur p-7 rounded- border border-emerald-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all">
          ...
          <div onClick={()=>setSelectedCard({ type:"conflict", title:"Bentrok Terdeteksi - Jam Tumpang Tindih" })} className={`group cursor-pointer p-7 rounded- border shadow-sm hover:-translate-y-1 transition-all ${conflictsCount>0? "bg-gradient-to-br from-red-50 to-rose-100 border-red-200" : "bg-white/80 border-emerald-100"}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-2 bg-white/80 backdrop-blur-xl rounded- border border-emerald-100 shadow-sm p-8">
            <div className="flex justify-between items-start mb-8">
              <div><h2 className="text-2xl font-bold text-emerald-950">Jumlah Jadwal per Konsultan</h2><p className="text-sm text-emerald-700/50 mt-1">Total schedules - last 30 days • Green Environment</p></div>
              <span className="text-xs px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full">Live</span>
            </div>
            <div className="space-y-4">
              {perKonsultan.map((item) => {
                const email = item[0];
                const count = item[1];
                return (
                  <div key={email} className="group">
                    <div className="flex justify-between text-xs mb-2"><span className="font-medium text-emerald-900 truncate max-w-">{email}</span><span className="font-bold">{count}</span></div>
                    <div className="h-3 bg-emerald-50 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-emerald-400 via-green-500 to-emerald-700 rounded-full" style={{ width: `${(count/maxCount)*100}%` }}></div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-gradient-to-br from-emerald-900 to-green-800 rounded- p-8 text-white shadow-xl">
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded- border border-emerald-100 shadow-sm p-6">
          <div className="flex flex-col lg:flex-row justify-between gap-4 mb-6">
            <div className="flex gap-2">
              ...
              {selectedCard && <button onClick={()=>setSelectedCard(null)} className="px-5 py-2.5 rounded-2xl text-sm bg-red-50 text-red-600">✕ Clear</button>}
            </div>
              ...
              <tbody>
                {filtered.map(s=>{
                  ...
                  <tr key={s.id} className={`border-t border-emerald-50 hover:bg-emerald-50/50 ${isConflict? "bg-red-50/70" : ""}`}>
                    <td className="py-4 px-4"><div className="font-semibold text-emerald-950">{formatTanggalHari(s.date)} {isConflict && <span className="ml-1 text- bg-red-500 text-white px-1.5 py-0.5 rounded-full">BENTROK</span>}</div><div className="text- text-zinc-400">{s.date}</div></td>
                    <td className="py-4 px-4"><div className="flex items-center gap-2"><div className="w-7 h-7 rounded-full bg-emerald-900 text-white flex items-center justify-center text- font-bold">{s.profiles?.email?.slice(0,2).toUpperCase()}</div><span className="text-xs">{s.profiles?.email}</span></div></td>
                    ...
                    <td className="py-4 px-4 text-xs">{s.start_time}-{s.end_time} <span className="ml-1 px-2 py-0.5 bg-emerald-50 text-emerald-700 rounded-full text-">{s.location}</span></td>
                    <td className="py-4 px-4"><span className={`px-2.5 py-1 rounded-full text- font-medium ${s.status==="PENDING"?"bg-amber-100 text-amber-800":s.status==="APPROVED"?"bg-emerald-100 text-emerald-800":"bg-red-100 text-red-800"}`}>{s.status}</span></td>
      ...
      {selectedCard && (
        <div className="fixed inset-0 bg-emerald-950/40 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={()=>setSelectedCard(null)}>
          <div className="bg-white rounded- w-full max-w-2xl max-h- overflow-hidden shadow-2xl border border-emerald-100" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-6 overflow-auto max-h- space-y-3">
              {detailData.map((s:any)=>(
                <div key={s.id} className="p-4 rounded-2xl border border-emerald-50 bg-emerald-50/30 flex justify-between items-center">
                  ...
                  <span className={`text- px-2.5 py-1 rounded-full ${s.status==="PENDING"?"bg-amber-100 text-amber-800":s.status==="APPROVED"?"bg-emerald-100 text-emerald-800":"bg-red-100 text-red-800"}`}>{s.status}</span>
                </div>
              ))}
