...
export default function GMDashboard(){
  ...
  const exportCSV = ()=>{
    const header = "Tanggal,Client,Project,Konsultan,Lokasi,Jam,Status\n";
    const rows = filtered.map(s=>`${s.date},${s.client_name},${s.project_name},${s.profiles?.email},${s.location},${s.start_time}-${s.end_time},${s.status}`).join("\n");
    ...
    <div className="min-h-screen bg-[#fcfbf8] p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white p-5 rounded-[20px] border"><div className="text-xs text-zinc-500">Total Konsultan</div><div className="text-2xl font-bold">{profiles.filter(p=>p.role==="CONSULTANT").length}</div></div>
          <div className="bg-white p-5 rounded-[20px] border"><div className="text-xs text-zinc-500">Jadwal Bulan Ini</div><div className="text-2xl font-bold">{schedules.length}</div></div>
          <div className="bg-amber-50 p-5 rounded-[20px] border border-amber-200"><div className="text-xs text-amber-700">Butuh Approval</div><div className="text-2xl font-bold text-amber-700">{pending.length}</div></div>
          <div className="bg-red-50 p-5 rounded-[20px] border border-red-200"><div className="text-xs text-red-700">Bentrok Terdeteksi</div><div className="text-2xl font-bold text-red-700">{conflicts}</div></div>
        </div>
        ...
        <div className="bg-white rounded-[24px] border p-6 mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-xs text-zinc-500"><tr><th className="text-left py-2">Tanggal</th><th className="text-left">Konsultan</th><th className="text-left">Client</th><th className="text-left">Project</th><th className="text-left">Jam</th><th className="text-left">Status</th><th>Aksi</th></tr></thead>
              <tbody>
                {filtered.map(s=>(
                  <tr key={s.id} className="border-t">
                    <td className="py-3">{s.date}</td><td>{s.profiles?.email}</td><td className="font-medium">{s.client_name}</td><td>{s.project_name}</td><td>{s.start_time}-{s.end_time} {s.location}</td>
